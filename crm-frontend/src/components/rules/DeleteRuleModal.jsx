...
export default function DeleteRuleModal({ open, rule, onClose, onConfirm }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-surface-800 rounded-3xl w-full max-w-sm mx-4 p-6 shadow-2xl animate-slide-up">
        <div className="text-center">
          ...
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-2">حذف قانون</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">
            آیا از حذف قانون <span className="font-semibold text-slate-900 dark:text-white">{rule.title}</span> اطمینان دارید؟
          </p>
